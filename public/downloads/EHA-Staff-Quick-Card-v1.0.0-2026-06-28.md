# Elevated Health Augusta — Staff Quick Reference Card

> Internal — print & laminate at front desk · v1.0.0 · 2026-06-28

**7013 Evans Town Center Blvd, Suite 203, Evans, GA 30809** · (706) 760-3470

## Opening script

We start with what you want, but we confirm with labs and a physician review before any prescription. The $79 Wellness Assessment is our front door for hormones, weight loss, and peptides — IV Lounge is walk-in without a consult.

## Lane A — IV Lounge

- Patient walks in or books IV Lounge online.
- IV screening edge function runs hard blocks (CHF, pregnancy, G6PD for high-dose vitamin C, etc.).
- Select drip or push; charge at checkout (signature drips $139–$169; NAD+ $50 booster; glutathione and other pushes).
- Members receive 20% off IV add-ons beyond the 2 included drips on ELEVATED IV.
- Soft mention $79 Wellness Assessment only if hormone/weight interest surfaces.

## Lane B — Consult-gated

- Patient pays $79 Wellness Assessment via Stripe before or at booking.
- In-person visit: history, vitals, goal confirmation.
- Order goal-appropriate lab panel; draw same visit when possible.
- Run CDS assessment in provider chart (staff) → physician review (prescriber only).
- Results call: enroll program, collect consents, schedule RN teaching.
- Never promise a specific medication before labs and physician sign-off.

## Patient journey

- **Arrival** (Front desk): Warm welcome, identify intent
- **Counsel & discover** (Kristen / Caroline): Educate on EHA services; no price guessing
- **Wellness Assessment** (Kristen + RN): Charge $79; intake + lane lock
- **Labs ordered & drawn** (Caroline): Correct panel, correct charge
- **Provider review** (Provider MD): Clinical decision within 5–7 days
- **Program enrollment** (Kristen): Stripe subscription + consents
- **Prescribe & ship** (Provider + Kristen): Vendor routing ALGO-003
- **Follow-up cadence** (Caroline + Kristen): RN check-ins, quarterly labs
- **Retain & grow** (All staff): Verify billing + ethical upsell

## ELEVATED programs

- **ELEVATED IV** $199/mo — 2 signature drips/mo · 20% off add-ons · priority booking
- **ELEVATED HRT** $229/mo — Cream only — no troches
- **ELEVATED TRT** $249/mo — No injections · no anastrozole · no HCG
- **ELEVATED GLP-1** $349/mo · $449/mo — Price locked to molecule — titration does not change monthly rate

See Formulary Cheat Sheet for complete SKU list.
