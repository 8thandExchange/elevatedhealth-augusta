 ...
 */
import {
  ALGO006_BASELINE_LABS,
  BASELINE_LABS_DRAW,
  BOOK_WELLNESS_ASSESSMENT_THEN,
  C01_CHARGE_VERIFY,
  C01_WELLNESS_AMOUNT,
  C02_COMPREHENSIVE_CHARGE,
  C02_EXPANDED_CHARGE,
  C02_LAB_AMOUNT,
  C03_PROGRAM_AMOUNT,
  C06_RETATRUTIDE_VERIFY,
  COMBO_ENROLL_EXAMPLE,
  COMPREHENSIVE_LAB_DEFAULT,
  COMPREHENSIVE_LAB_ORDER,
  CONSULT_CREDIT_NOTE,
  ELEVATED_IV_OFFER_THEN,
  EXPANDED_LAB_ORDER,
  EXPANDED_LAB_WEIGHT_SLUG,
  GLP1_ALGO006_ENROLL,
  GLP1_COUNSEL_THEN,
  GLP1_ENROLL_THEN,
  ...
  HORMONE_COUNSEL_THEN,
  HRT_ENROLL_THEN,
  HRT_SERMORELIN_CHARGE,
  IV_LOUNGE_ADDON_CHARGE,
  IV_WALKIN_ASSESSMENT_SCRIPT,
  IV_WALKIN_ASSESSMENT_UPSELL,
  JOURNEY_CONSULT_GOAL,
  LAB_PANEL_CHARGE_RANGE,
  LANE_B_CONSULT_CHARGE,
  PEPTIDE_IV_COUNSEL_THEN,
  RECOVERY_FILL_CHARGE,
  RESULTS_REVIEW_HORMONE_GLP_CHARGE,
  REVENUE_LANE_BULLETS,
  TRT_ENROLL_THEN,
  WELLNESS_ASSESSMENT_BOOK_THEN,
  WELLNESS_ASSESSMENT_COMPLETED,
  WELLNESS_ASSESSMENT_IV_MENTION,
...
export const PATIENT_JOURNEY_PHASES: JourneyPhase[] = [
  ...
  { id: "consult", label: "2", title: "Wellness Assessment", owner: "Kristen + RN", goal: JOURNEY_CONSULT_GOAL },
...
export const UPSELL_MATRIX: UpsellRow[] = [
  {
    ...
    script: IV_WALKIN_ASSESSMENT_SCRIPT,
  },
  {
    phase: "Counseling",
...
export const SOP_ALGORITHMS: SOPAlgorithm[] = [
  {
    ...
    steps: [
      ...
      },
      {
        ...
        then: PEPTIDE_IV_COUNSEL_THEN,
      },
      {
        step: "1.6",
        phase: "Counsel & discover",
        if: "IV hydration, immunity, hangover, NAD+ interest only",
        then: WELLNESS_ASSESSMENT_IV_MENTION,
      },
      {
        step: "1.7",
        phase: "Counsel & discover",
      ...
      // Phase 2 — Book consult
      {
        ...
        then: BOOK_WELLNESS_ASSESSMENT_THEN,
        charge: C01_CHARGE_VERIFY,
      },
      {
        step: "2.2",
        phase: "Wellness Assessment",
        action: CONSULT_CREDIT_NOTE,
      },
      {
        step: "2.3",
        phase: "Wellness Assessment",
      ...
      // Phase 3 — Labs
      {
        ...
        then: COMPREHENSIVE_LAB_ORDER,
        charge: C02_COMPREHENSIVE_CHARGE,
      },
      {
        step: "3.2",
        phase: "Labs",
        if: "GLP-1 candidacy",
        then: EXPANDED_LAB_ORDER,
        charge: C02_EXPANDED_CHARGE,
      },
      {
        step: "3.3",
        phase: "Labs",
        if: "Advanced recomposition candidacy (GLP-1 lane)",
        then: EXPANDED_LAB_WEIGHT_SLUG,
        charge: C02_EXPANDED_CHARGE,
      },
      {
        step: "3.4",
        ...
        then: COMPREHENSIVE_LAB_DEFAULT,
      },
      {
        step: "3.5",
        phase: "Labs",
      ...
      },
      {
        ...
        upsell: IV_WALKIN_ASSESSMENT_UPSELL,
      },
      {
        step: "8.IV.3",
        phase: "IV Lounge",
  ...
  },
  {
    ...
    steps: [
      { step: "1", action: "Ask: What brings you in today?" },
      {
        ...
        then: "→ Lane A: /iv-lounge. Book IV slot. No " + WELLNESS_ASSESSMENT_PRICE + " consult required.",
        else: "→ Continue to step 3",
      },
      {
        step: "3",
        if: "Patient mentions hormones, weight loss, peptides, or metabolic stack",
        then: WELLNESS_ASSESSMENT_BOOK_THEN,
  ...
  },
  {
    ...
    purpose: `Standard path from ${WELLNESS_ASSESSMENT_PRICE} consult to recurring program after labs.`,
    owner: "Kristen + Provider MD",
    steps: [
      { step: "1", action: WELLNESS_ASSESSMENT_COMPLETED },
      { step: "2", action: BASELINE_LABS_DRAW },
      { step: "3", action: "Provider reviews labs within 5–7 business days." },
      {
        step: "4",
        if: "Male + low T pattern + shared decision",
        then: TRT_ENROLL_THEN,
      },
      {
        step: "5",
        if: "Female + BHRT candidacy",
        then: HRT_ENROLL_THEN,
      },
      {
        step: "6",
        if: "GLP-1 candidacy (semaglutide or tirzepatide)",
  ...
  },
  {
    ...
    steps: [
      ...
      { step: "2", action: ALGO006_BASELINE_LABS },
